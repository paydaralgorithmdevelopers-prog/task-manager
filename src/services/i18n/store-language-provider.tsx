...
function StoreLanguageProvider(props: PropsWithChildren<{}>) {
  const [language, setLanguageRaw] = useState<Language>(
    () => (Cookies.get(cookieName) ?? fallbackLanguage) as Language
  );

  const setLanguage = useCallback((language: Language) => {
    Cookies.set(cookieName, (language ?? fallbackLanguage) as string);
    setLanguageRaw((language ?? fallbackLanguage) as Language);
  }, []);
